...
// Dados temporários por usuário (ex: nome completo antes de salvar agendamento)
...
export const handler = async (event) => {
    ...
    try {
        ...
        const incomingMsg = twilioData.Body
            ? twilioData.Body.toLowerCase().trim()
            : "";
...
async function salvarAgendamento(dadosAgendamento) {
    const params = {
        ...
        Item: {
            ...
            timestamp_brasil: new Date().toLocaleString("pt-BR", {
...
async function processMessage(message, telefone, nomeCliente, estadoAtual) {
    ...
    const mensagemNormalizada = message
        .normalize("NFD")
        .replace(/[\u0300-\u036f]/g, "")
        .toLowerCase();

    // LÓGICA SIMPLIFICADA - MENOS ESTADOS
    if (estadoAtual === "aguardando_nome") {
        // Captura o nome completo enviado pelo usuário e passa para o próximo passo
        const nomeCompleto = message.trim();
        userData.set(telefone, { nomeCompleto });
        response = `Obrigado, ${nomeCompleto}.
Por favor, agora envie os dados do agendamento no formato:\n\n*Data* + *Horario* + *Servico*\n\nEx: "25/03 14h - Manicure completa"`;
        novoEstado = "aguardando_agendamento";
    } else if (
        estadoAtual === "aguardando_agendamento" &&
        contemDadosAgendamento(mensagemNormalizada)
    ) {
        // Usuário enviou dados de agendamento
        const nomeDaSessao = userData.get(telefone)?.nomeCompleto || nomeCliente;
        const resultado = processarDadosAgendamento(message, telefone, nomeDaSessao);
        response = resultado.response;
        agendamentoParaSalvar = resultado.dados;
        novoEstado = "menu_principal"; // Volta ao menu
        // Limpa dados temporários do usuário
        userData.delete(telefone);
    } else if (
        mensagemNormalizada.includes("1") ||
        mensagemNormalizada.includes("agendar")
    ) {
        // Usuário quer agendar
        // Primeiro peça o nome completo para usar no agendamento
        response = `Antes de agendar, por favor envie seu *nome completo*.`;
        novoEstado = "aguardando_nome";
    } else {
        // Menu principal ou comandos simples
        response = await processarMenuPrincipal(mensagemNormalizada, nomeCliente, telefone);
...
function contemDadosAgendamento(mensagem) {
    return (
        /(\d{1,2}\/\d{1,2})|(amanha|hoje|segunda|terca|quarta|quinta|sexta|sabado)/.test(
            mensagem
...
async function processarMenuPrincipal(mensagem, nomeCliente, telefone) {
    const displayName = userData.get(telefone)?.nomeCompleto || nomeCliente;
    if (
        mensagem.includes("oi") ||
        mensagem.includes("ola") ||
        mensagem.includes("menu")
    ) {
        return `Ola ${displayName}! Sou o assistente da Esmalteria! 💅
...
Digite o numero da opcao desejada!`;
    } else if (mensagem.includes("2") || mensagem.includes("servico")) {
...
function parseTwilioData(event) {
    ...
    if (event.body) {
        ...
        pairs.forEach((pair) => {
            ...
            if (key && value) {
                parsed[decodeURIComponent(key)] = decodeURIComponent(
                    value.replace(/\+/g, " ")
                );
            }
        });
